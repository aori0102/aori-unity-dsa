    ...
    public class MaxHeap<T> : IEnumerable<T> where T : IComparable<T>
    {
        ...
        private void HeapifyDown(int index)
        {
            ...
            while (true)
            {
                ...
                if (left <= last && r_elements[left].CompareTo(r_elements[greatest]) > 0)
                {
                    greatest = left;
                }

                if (right <= last && r_elements[right].CompareTo(r_elements[greatest]) > 0)
                {
                    greatest = right;
                }
                ...
                (r_elements[index], r_elements[greatest]) = (r_elements[greatest], r_elements[index]);
                index = greatest;
            }
        }
    }
